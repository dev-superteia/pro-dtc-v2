...
class RawMaterialAndCompoundRepository():
    ...
    async def totalComponents(db: AsyncSession, type, plant, market_segment, line, year, type_selected):
        filter_plant = f"""and mez.plant = '{plant}'""" if plant else ''
        filter_plant_raw = f"""and plant = '{plant}'""" if plant else ''
        filter_plant_comp = f"""and comp.plant = '{plant}'""" if plant else ''
        filter_line = f"""and ms.line = '{line}'""" if line else ''
        ...
        if type == 'raw':  
            ...
            filter_type = f"""and mez.component = '{type_selected}'""" if type_selected else ''   
            query = f"""with mass as(select mx.material,mez.plant, mx.mat_desc,mez.month,sum(mez.utilization),
                        coalesce((select sum(mv.total_volum)as volume from material_volum mv where material = mx.material and  mv.plant = mez.plant and month = mez.month group by material),0)                   
                        from (select mm.material, mm.mat_desc, ms.line from md_material mm left join market_segment
                        ms on mm.mkg_segm = ms.mkg_segm where
                        mm.subclass = 'IPAA'{filter_line}) as mx, material_espec_zp45 mez where mez.material = mx.material 
                        {filter_plant} {filter_type} and mez.year = """+year+""" group by mx.material, plant, mx.line, mx.mat_desc,mez."month")
                        select plant ,sum(sum*"coalesce")as total from mass mt group by plant, month
                        """
